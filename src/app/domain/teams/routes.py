...
from ...lib.guards import route_guards
from ...lib.schema import TeamsPlayers
from ...server.auth import AuthenticationMiddleware


@post(path="/players", guards=[route_guards])
async def post_players(
    data: TeamsPlayers, db_connection: Connection, scope: Scope, request: Request
) -> bool:  # noqa: ARG001
    async with db_connection.transaction():
        ...
        query = """
            ...
            returning id
        """
        team = await db_connection.fetchrow(
            query, data.name, data.price, protocol, user["name"]
        )  # type: ignore[index]
        if team:
            rows = [(i.name, team["id"]) for i in data.players]
            await db_connection.executemany(
                "insert into players (name, team_id) values ($1, $2)", rows
            )
...
async def get_players(db_connection: Connection) -> list[TeamsPlayers]:
    ...
    if teams:
        for t in teams:
            ...
            players = await db_connection.fetch(
                "select * from players where team_id = $1", t["id"]
            )
            for p in players:
                player = {
                    "id": p["id"],
